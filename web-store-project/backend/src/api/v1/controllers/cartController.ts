...
export async function getCart(req: Request, res: Response) {
    try {
        const cart = await fetchCart(req.params.cartId[0]);
...
export async function addToCart(req: Request, res: Response) {
    try {
        ...
        const item = await addItemToCart(req.params.cartId[0], partId);
...
export async function updateCartItem(req: Request, res: Response) {
    try {
        ...
        const item = await updateItemQuantity(req.params.itemId[0], quantity);
...
export async function removeCartItem(req: Request, res: Response) {
    try {
        await removeItemFromCart(req.params.itemId[0]);
...
export async function clearCart(req: Request, res: Response) {
    try {
        await clearCartItems(req.params.cartId[0]);
